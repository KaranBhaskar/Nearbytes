...
function generateFallbackTags(name) {
  ...
  const tag2 = FALLBACK_TAG_POOL[idx2 === idx1 ? (idx2 + 1) % FALLBACK_TAG_POOL.length : idx2];
  return [tag1, tag2];
}
...
app.get("/runtime-config.js", (_req, res) => {
  ...
  res.send(`window.__APP_CONFIG__ = Object.freeze(${JSON.stringify(config, null, 2)});\n`);
});
...
function normalizeTagList(value) {
  ...
  return String(value || "").split(/[;,]/).map((t) => String(t).trim()).filter(Boolean);
}
...
function inferExternalDietaryTags(tags = {}) {
  ...
  if (hasTruthyDietaryValue(tags["diet:vegan"]) || combinedText.includes("vegan")) dietaryTags.add("vegan");
  if (hasTruthyDietaryValue(tags["diet:vegetarian"]) || combinedText.includes("vegetarian")) dietaryTags.add("vegetarian");
  if (hasTruthyDietaryValue(tags["diet:halal"]) || combinedText.includes("halal")) dietaryTags.add("halal");
  if (hasTruthyDietaryValue(tags["diet:kosher"]) || combinedText.includes("kosher")) dietaryTags.add("kosher");
  if (
  ...
  if (combinedText.includes("salad") || combinedText.includes("veggie") || combinedText.includes("plant-based")) dietaryTags.add("vegetarian");
  if (combinedText.includes("indian") || combinedText.includes("mediterranean") || combinedText.includes("middle eastern") || combinedText.includes("falafel")) dietaryTags.add("vegetarian");
  if (combinedText.includes("shawarma") || combinedText.includes("kebab") || combinedText.includes("gyro") || combinedText.includes("halal")) dietaryTags.add("halal");
...
function normalizeExternalRestaurantElement(element, originLat, originLng) {
  ...
  if (lowerName.includes("subway")) inferredDietaryTags = [...new Set([...inferredDietaryTags, "vegetarian"])];
  if (lowerName.includes("osmow") || lowerName.includes("shawarma")) inferredDietaryTags = [...new Set([...inferredDietaryTags, "halal"])];

  let cuisineTags = normalizeTagList(tags.cuisine);
  // Ensure at least 2 cuisine tags
  if (cuisineTags.length < 2) cuisineTags = generateFallbackTags(name);
...
async function fetchGooglePlacesNearby(lat, lng, radiusMeters = 5000) {
  ...
  try {
    ...
    const body = {
      includedTypes: ["restaurant", "cafe", "fast_food_restaurant", "food_court"],
      maxResultCount: 20,
      locationRestriction: {
        circle: {
    ...
    const response = await fetch(url.toString(), {
      ...
      headers: {
        ...
        "X-Goog-FieldMask": "places.id,places.displayName,places.formattedAddress,places.location,places.rating,places.userRatingCount,places.regularOpeningHours,places.websiteUri,places.internationalPhoneNumber,places.types,places.primaryTypeDisplayName",
      },
    ...
    if (!response.ok) {
      ...
      console.warn("Google Places API error:", response.status, errText.slice(0, 200));
      return null;
    }
...
function normalizeGooglePlace(place, originLat, originLng) {
  ...
  const types = (place.types || []).filter(
    (t) => !["establishment", "point_of_interest", "food", "restaurant"].includes(t)
  );
...
`;
    ...
    normalizedRestaurants = (data.elements || [])
      ...
      .filter(Boolean)
      .filter((restaurant) => {
        const key = restaurant.externalPlaceId || `${restaurant.name.toLowerCase()}|${restaurant.address.toLowerCase()}`;
  ...
  return normalizedRestaurants.sort((a, b) => a.distanceKm - b.distanceKm).slice(0, 60);
}
...
async function syncNearbyRestaurants(lat, lng) {
  ...
  if (GOOGLE_MAPS_API_KEY) {
    ...
    console.warn("Google Places returned no results, falling back to OpenStreetMap.");
  }
...
function upsertExternalRestaurant(restaurant) {
  ...
  if (existing) {
    ...
    `).run(
      ...
      restaurant.description, restaurant.phone, restaurant.website, restaurant.openingHours,
      JSON.stringify(restaurant.cuisineTags), JSON.stringify(restaurant.dietaryTags),
      restaurant.googleRating || null, restaurant.googleRatingCount || null,
      existing.id
    );
    return existing.id;
  }
  ...
  `).run(
    null, restaurant.name, restaurant.address, restaurant.lat, restaurant.lng,
    restaurant.description, restaurant.phone, restaurant.website, restaurant.openingHours,
    JSON.stringify(restaurant.cuisineTags), JSON.stringify(restaurant.dietaryTags),
    restaurant.externalPlaceId, restaurant.googleRating || null, restaurant.googleRatingCount || 0
  );
...
function normalizeRestaurantRow(row) {
  ...
  // Ensure at least 2 cuisine tags on every restaurant
  if (cuisineTags.length < 2) {
    cuisineTags = generateFallbackTags(row.name || "");
  }

  const isOsm = row.google_place_id && String(row.google_place_id).startsWith("osm:");
  const isGoogle = row.google_place_id && String(row.google_place_id).startsWith("google:");
  ...
  return {
    ...
    source: isOsm ? "openstreetmap" : isGoogle ? "google" : row.owner_id ? "manual" : "seed",
...
function canManageRestaurant(user, restaurant) {
  return user.role === "moderator" || Number(restaurant.owner_id) === Number(user.id);
}
...
function parseRestaurantPayload(body, existing = null) {
  ...
  const tagsSource = rawCuisineTags == null && existing ? parseCuisineTags(existing.cuisine_tags) : rawCuisineTags;
  let cuisineTags = Array.isArray(tagsSource)
    ? tagsSource.map((t) => String(t).trim()).filter(Boolean)
    : String(tagsSource || "").split(",").map((t) => t.trim()).filter(Boolean);

  if (cuisineTags.length < 2) cuisineTags = generateFallbackTags(nextName);

  const dietarySource = rawDietaryTags == null && existing ? parseCuisineTags(existing.dietary_tags) : rawDietaryTags;
  const dietaryTags = Array.isArray(dietarySource)
    ? dietarySource.map((t) => String(t).trim().toLowerCase()).filter(Boolean)
    : String(dietarySource || "").split(",").map((t) => t.trim().toLowerCase()).filter(Boolean);
  ...
  return {
    values: {
      ...
      lat: nextLat,
      lng: nextLng,
      description: rawDescription == null ? (existing ? existing.description : null) : String(rawDescription).trim() || null,
      phone: rawPhone == null ? (existing ? existing.phone : null) : String(rawPhone).trim() || null,
      website: rawWebsite == null ? (existing ? existing.website : null) : String(rawWebsite).trim() || null,
      openingHours: rawOpeningHours == null ? (existing ? existing.opening_hours : null) : String(rawOpeningHours).trim() || null,
      menuUrl: rawMenuUrl == null ? (existing ? existing.menu_url : null) : safeHttpUrl(rawMenuUrl),
      cuisineTags,
      dietaryTags,
    },
  };
...
function updateRestaurantRecord(restaurantId, values) {
  db.prepare(`
    ...
    WHERE id = ?
  `).run(
    values.name, values.address, values.lat, values.lng, values.description,
    values.phone, values.website, values.openingHours, values.menuUrl,
    JSON.stringify(values.cuisineTags), JSON.stringify(values.dietaryTags),
    restaurantId
  );
}
...
async function enrichTagsWithGemini(restaurants) {
  if (!GEMINI_API_KEY || !restaurants.length) {
    ...
    return restaurants.map((r) => ({
      restaurantId: r.id,
      cuisineTags: r.cuisineTags && r.cuisineTags.length >= 2 ? r.cuisineTags : generateFallbackTags(r.name),
...
${restaurants.map((r) => `- id:${r.id} name:"${r.name}" address:"${r.address}"`).join("\n")}`;
    ...
    return parsed.map((item) => ({
      ...
      cuisineTags: Array.isArray(item.cuisineTags) ? item.cuisineTags : generateFallbackTags(item.id),
...
app.get("/api/location/search", async (req, res) => {
  ...
  try {
    ...
    return res.json({
      ...
      boundingBox: Array.isArray(best.boundingbox) ? best.boundingbox.map(Number) : null,
    });
  } catch (err) {
...
app.get("/api/location/reverse", async (req, res) => {
    ...
    try {
      ...
      if (data.status === "OK" && data.results?.[0]) {
        ...
        const cityComp = result.address_components?.find(
          (c) => c.types.includes("locality") || c.types.includes("postal_town") || c.types.includes("administrative_area_level_2")
        );
        return res.json({
  ...
  try {
    ...
    return res.json({
      label: address.city || address.town || address.village || address.state || data.display_name || "Unknown Location",
    });
  } catch (err) {
...
app.get("/api/location/restaurants", async (req, res) => {
  ...
  try {
    const items = (await syncNearbyRestaurants(lat, lng)).map((r) => ({
      ...
      externalSource: r.externalPlaceId?.startsWith("google:") ? "google" : "openstreetmap",
    }));
    return res.json({ items });
  } catch (err) {
    return res.status(502).json({ error: err.message || "Failed to load nearby restaurants" });
  }
});
...
app.post("/api/auth/signup", (req, res) => {
  ...
  if (!["customer", "owner", "moderator"].includes(role)) {
    return res.status(400).json({ error: "role must be customer, owner, or moderator" });
  }
  ...
  const user = db.prepare("SELECT id, name, email, role, is_banned, banned_at, banned_reason FROM users WHERE id = ?")
...
app.post("/api/auth/login", (req, res) => {
  ...
  if (user.is_banned) {
    return res.status(403).json({ error: user.banned_reason || "This account has been suspended." });
  }
...
app.get("/api/restaurants/nearby", async (req, res) => {
  ...
  const withDistance = rows
    .map((row) => ({ ...normalizeRestaurantRow(row), distanceKm: haversineKm(lat, lng, row.lat, row.lng) }))
    .filter((r) => dietaryFilters.every((tag) => r.dietaryTags.map((t) => t.toLowerCase()).includes(tag)))
    .sort((a, b) => a.distanceKm - b.distanceKm);
...
app.post("/api/restaurants/refine-tags", async (req, res) => {
  ...
  const restaurants = restaurantIds
    .map((id) => db.prepare("SELECT id, name, address, cuisine_tags, dietary_tags FROM restaurants WHERE id = ?").get(Number(id)))
    .filter(Boolean)
    .map((r) => ({ ...r, cuisineTags: parseCuisineTags(r.cuisine_tags), dietaryTags: parseCuisineTags(r.dietary_tags) }));

  const updates = await enrichTagsWithGemini(restaurants);

  // Persist the enriched tags
  const updateStmt = db.prepare("UPDATE restaurants SET cuisine_tags = ?, dietary_tags = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?");
  for (const update of updates) {
    const id = Number(update.restaurantId);
    if (!Number.isFinite(id)) continue;
    updateStmt.run(JSON.stringify(update.cuisineTags), JSON.stringify(update.dietaryTags), id);
  }
...
app.put("/api/restaurants/:id/reviews/:reviewId", requireAuth, requireRole("customer"), (req, res) => {
  ...
  if (!Number.isFinite(restaurantId) || !Number.isFinite(reviewId)) return res.status(400).json({ error: "invalid id" });
  if (!Number.isFinite(rating) || rating < 1 || rating > 5) return res.status(400).json({ error: "rating must be between 1 and 5" });

  const review = db.prepare("SELECT * FROM reviews WHERE id = ? AND restaurant_id = ?").get(reviewId, restaurantId);
  if (!review) return res.status(404).json({ error: "review not found" });
  if (review.user_id !== req.user.id) return res.status(403).json({ error: "you can only edit your own review" });

  db.prepare("UPDATE reviews SET rating = ?, comment = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
    .run(Math.round(rating), comment, reviewId);

  const updated = db.prepare("SELECT id, rating, comment, created_at AS createdAt, updated_at AS updatedAt FROM reviews WHERE id = ?").get(reviewId);
  return res.json({ review: updated });
});

app.delete("/api/restaurants/:id/reviews/:reviewId", requireAuth, requireRole("customer", "moderator"), (req, res) => {
  const restaurantId = Number(req.params.id);
  const reviewId = Number(req.params.reviewId);
  if (!Number.isFinite(restaurantId) || !Number.isFinite(reviewId)) return res.status(400).json({ error: "invalid id" });
...
app.delete("/api/reviews/:reviewId", requireAuth, requireRole("customer", "moderator"), (req, res) => {
  const reviewId = Number(req.params.reviewId);
...
app.get("/api/owner/restaurants", requireAuth, requireRole("owner", "moderator"), (req, res) => {
  const whereClause = req.user.role === "moderator" ? "" : "WHERE r.owner_id = ?";
...
app.post("/api/owner/restaurants", requireAuth, requireRole("owner", "moderator"), (req, res) => {
  ...
  `).run(
    req.user.id, values.name, values.address, values.lat, values.lng,
    values.description, values.phone, values.website, values.openingHours, values.menuUrl,
    JSON.stringify(values.cuisineTags), JSON.stringify(values.dietaryTags),
    null, null, 0
  );
...
app.put("/api/owner/restaurants/:id", requireAuth, requireRole("owner", "moderator"), (req, res) => {
  const restaurantId = Number(req.params.id);
  if (!Number.isFinite(restaurantId)) return res.status(400).json({ error: "invalid restaurant id" });

  const existing = db.prepare("SELECT * FROM restaurants WHERE id = ?").get(restaurantId);
...
app.delete("/api/owner/restaurants/:id", requireAuth, requireRole("owner", "moderator"), (req, res) => {
  const restaurantId = Number(req.params.id);
  if (!Number.isFinite(restaurantId)) return res.status(400).json({ error: "invalid restaurant id" });

  const existing = db.prepare("SELECT id, owner_id FROM restaurants WHERE id = ?").get(restaurantId);
  if (!existing || !canManageRestaurant(req.user, existing)) {
    return res.status(404).json({ error: "owned restaurant not found" });
  }
...
app.post("/api/owner/restaurants/:id/images", requireAuth, requireRole("owner", "moderator"), upload.array("images", 10), (req, res) => {
  const restaurantId = Number(req.params.id);
  if (!Number.isFinite(restaurantId)) return res.status(400).json({ error: "invalid restaurant id" });
  ...
  if ((!req.files || req.files.length === 0) && !imageUrl) {
    return res.status(400).json({ error: "at least one image URL or file is required" });
  }

  const hasCover = db.prepare("SELECT id FROM restaurant_images WHERE restaurant_id = ? AND is_cover = 1").get(restaurantId);
  const insert = db.prepare("INSERT INTO restaurant_images(restaurant_id, url, is_cover) VALUES (?, ?, ?)");
  const inserted = [];

  if (imageUrl) {
    const isCover = hasCover ? 0 : 1;
    const info = insert.run(restaurantId, imageUrl, isCover);
    inserted.push({ id: info.lastInsertRowid, url: imageUrl, isCover: Boolean(isCover) });
  }
...
app.post("/api/owner/restaurants/:id/menu-items", requireAuth, requireRole("owner", "moderator"), upload.single("image"), (req, res) => {
  const restaurantId = Number(req.params.id);
  if (!Number.isFinite(restaurantId)) return res.status(400).json({ error: "invalid restaurant id" });

  const owned = db.prepare("SELECT id, owner_id FROM restaurants WHERE id = ?").get(restaurantId);
  ...
  const imageUrl = req.file ? `/uploads/${req.file.filename}` : safeHttpUrl(req.body.imageUrl);
  const info = db.prepare(
    "INSERT INTO menu_items(restaurant_id, name, description, price, image_url) VALUES (?, ?, ?, ?, ?)"
  ).run(restaurantId, String(name).trim(), description ? String(description).trim() : null, parsedPrice, imageUrl);
...
app.put("/api/owner/restaurants/:id/menu-items/:itemId", requireAuth, requireRole("owner", "moderator"), upload.single("image"), (req, res) => {
  const restaurantId = Number(req.params.id);
  const itemId = Number(req.params.itemId);
  if (!Number.isFinite(restaurantId) || !Number.isFinite(itemId)) return res.status(400).json({ error: "invalid ids" });

  const owned = db.prepare("SELECT id, owner_id FROM restaurants WHERE id = ?").get(restaurantId);
  if (!owned || !canManageRestaurant(req.user, owned)) return res.status(404).json({ error: "owned restaurant not found" });

  const existingItem = db.prepare("SELECT * FROM menu_items WHERE id = ? AND restaurant_id = ?").get(itemId, restaurantId);
  ...
  const nextDescription = req.body.description == null ? existingItem.description : String(req.body.description).trim();
  const nextPrice = req.body.price == null ? existingItem.price : (Number.isFinite(Number(req.body.price)) ? Number(req.body.price) : existingItem.price);
  if (!Number.isFinite(nextPrice) || nextPrice < 0) return res.status(400).json({ error: "price must be a non-negative number" });

  const nextImage = req.file ? `/uploads/${req.file.filename}` : safeHttpUrl(req.body.imageUrl) || existingItem.image_url;

  db.prepare("UPDATE menu_items SET name = ?, description = ?, price = ?, image_url = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?")
...
app.delete("/api/owner/restaurants/:id/menu-items/:itemId", requireAuth, requireRole("owner", "moderator"), (req, res) => {
  const restaurantId = Number(req.params.id);
  const itemId = Number(req.params.itemId);
  if (!Number.isFinite(restaurantId) || !Number.isFinite(itemId)) return res.status(400).json({ error: "invalid ids" });
  ...
  const existingItem = db.prepare("SELECT id FROM menu_items WHERE id = ? AND restaurant_id = ?").get(itemId, restaurantId);
  if (!existingItem) return res.status(404).json({ error: "menu item not found" });
...
app.put("/api/owner/restaurants/:id/visibility", requireAuth, requireRole("moderator"), (req, res) => {
  const restaurantId = Number(req.params.id);
  if (!Number.isFinite(restaurantId)) return res.status(400).json({ error: "invalid restaurant id" });
...
app.get("/api/moderation/users", requireAuth, requireRole("moderator"), (_req, res) => {
...
app.put("/api/moderation/users/:id/ban", requireAuth, requireRole("moderator"), (req, res) => {
  const userId = Number(req.params.id);
  if (!Number.isFinite(userId)) return res.status(400).json({ error: "invalid user id" });
  if (userId === Number(req.user.id)) return res.status(400).json({ error: "Moderators cannot suspend themselves." });
...
app.delete("/api/moderation/users/:id", requireAuth, requireRole("moderator"), (req, res) => {
  const userId = Number(req.params.id);
  if (!Number.isFinite(userId)) return res.status(400).json({ error: "invalid user id" });
  if (userId === Number(req.user.id)) return res.status(400).json({ error: "Moderators cannot delete themselves." });

  const existing = db.prepare("SELECT id, role FROM users WHERE id = ?").get(userId);
  if (!existing) return res.status(404).json({ error: "user not found" });
  if (existing.role === "moderator") return res.status(400).json({ error: "Moderators cannot delete other moderators." });
